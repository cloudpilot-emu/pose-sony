...
{
	CALLED_SETUP ("Err", "UInt8 operation, UInt32* widthP"
						", UInt32* heightP, UInt32* depthP"
	);

	CALLED_GET_PARAM_VAL (UInt8, operation);
	CALLED_GET_PARAM_VAL (UInt32, depthP);
...
//		|| gSession->GetDevice().GetDeviceType() == kDeviceYSX1100)
	{
		...
		if (winH)
		{
			...
			EmMem_memcpy((void*)&state, stateP, sizeof(DrawStateType));
			...
			EmMem_memset(begin, data, (end-begin));
		}
	}
}
#endif //SONY_ROM
