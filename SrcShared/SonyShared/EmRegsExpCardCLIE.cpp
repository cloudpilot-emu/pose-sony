...
#define INSTALL_HANDLER(read, write, reg)			\
					this->SetHandler ((ReadFunction) &EmRegsExpCardCLIE::read,		\
									  (WriteFunction) &EmRegsExpCardCLIE::write,		\
									  addressof (reg),			\
									  sizeof (fRegs.reg))
#pragma mark -
...
void EmRegsExpCardCLIE::Initialize (void)
{
	EmRegs::Initialize ();
}
...
/***********************************************************************
 *
 * FUNCTION:	EmRegsExpCardCLIE::Save
 *
...
void EmRegsExpCardCLIE::Save (SessionFile& f)
{
	EmRegs::Save (f);
}
...
/***********************************************************************
 *
 * FUNCTION:	EmRegsExpCardCLIE::Load
 *
...
void EmRegsExpCardCLIE::Load (SessionFile& f)
{
	EmRegs::Load (f);
}
...
{
	if (fBaseAddr)
		return fBaseAddr + fAddressOffset;
...
uint32 EmRegsExpCardCLIE::GetAddressRange (void)
{
	return sizeof(fRegs);
}
...
// ---------------------------------------------------------------------------
//		¥ EmRegsExpCardCLIE::ValidAddress
// ---------------------------------------------------------------------------

int EmRegsExpCardCLIE::ValidAddress (emuptr address, uint32 iSize)
{
	UNUSED_PARAM(iSize)
	return 1;
}
...
// ---------------------------------------------------------------------------
//		¥ EmRegsExpCardCLIE::InvalidAccess
// ---------------------------------------------------------------------------

void EmRegsExpCardCLIE::InvalidAccess (emuptr address, long size, Bool forRead)
{
	EmAssert (gCPU68K);
...
// ---------------------------------------------------------------------------
//		¥ EmRegsExpCardCLIE::AddressError
// ---------------------------------------------------------------------------

void EmRegsExpCardCLIE::AddressError (emuptr address, long size, Bool forRead)
{
	EmAssert (gCPU68K);
...
void EmRegsExpCardCLIE::UnsupportedWrite (emuptr address, int size, uint32 value)
{
	...
	{
		EmRegsExpCardCLIE::InvalidAccess (address, size, false);
	}
}
