...
#include "EmPatchModule_ExpMgr.h"	
...
/***********************************************************************
 *
 * FUNCTION:	EmPatchModule_ExpMgr::EmPatchModule_ExpMgr
 *
 ...
 *
 ***********************************************************************/
EmPatchModule_ExpMgr::EmPatchModule_ExpMgr() :
	EmPatchModule("~ExpMgr"/*不明*/, gProtoExpMgrPatchTable)
{
}
...
CallROMType ExpMgrLibHeadpatch::CardPresent (void)
{
	PRINTF ("----------------------------------------");
	PRINTF ("CardPresent");
	CALLED_SETUP("Err", "UInt16 slotNumber");

	CALLED_GET_PARAM_VAL(UInt16, slotNumber);
...
CallROMType ExpMgrLibHeadpatch::CardRemoved (void)
{
	PRINTF ("----------------------------------------");
	PRINTF ("CardRemoved");
	CALLED_SETUP("Err", "UInt16 slotRefNum");
...
CallROMType ExpMgrLibHeadpatch::CardInfo(void)
{
	PRINTF ("----------------------------------------");
	PRINTF ("CardInfo");
	CALLED_SETUP("Err", "UInt16 slotNumber, ExpCardInfoType* infoP");
...
CallROMType ExpMgrLibHeadpatch::SlotEnumerate (void)
{
	...
	CALLED_SETUP("Err", "UInt16* slotRefNumP, UInt32* slotIteratorP");

	CALLED_GET_PARAM_REF(UInt16,		slotRefNumP,	Marshal::kInOut);
	CALLED_GET_PARAM_REF(UInt32,		slotIteratorP,	Marshal::kInOut);
