...
#define gSerialManagerVersion fred
	...
	err = FtrGet (sysFileCSystem, sysFtrNumROMVersion, &ftrValue);

	ErrFatalDisplayIf (err, "Unable to determine System version");

	if (sysGetROMVerMajor(ftrValue) < 2)
	{
	...
	else
	{
		...
		else
		{
			...
			{
				gSerialManagerVersion = ftrValue + 2;
			}
		}
	}

	StrPrintF (buffer, "%d  %d", (int) err, (int) ftrValue);
